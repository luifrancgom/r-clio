...
Este capítulo se basa en [@wickham_ggplot2_2016] y [@wilke_fundamentals_2019] así como en la documentación oficial de [`ggplot2`]({{< var url.packages.ggplot2 >}}) donde tiene como propósito introducir el modelo mental de **la gramática de gráficos en capas** (**layered grammar of graphics**, [@wickham_layered_2010]) para visualizar variables numéricas a través de un conjunto de categorías utilizando el paquete `ggplot2` e información de los resultados del examen [Saber Pro]({{< var url.institution.icfes_saber_pro >}}) del grupo de competencias genéricas correspondientes a programas de pregrado de la Facultad de Estudios a Distancia (FAEDIS) de la Universidad Militar Nueva Granada (UMNG).
...
Para desarrollar el contenido de este capítulo es necesario que descargues el archivo señalado a continuación:
   
::: {.callout appearance="minimal"}
## ![](images/download-solid-full.png){fig-alt="Ícono del explorador de archivos" height=25} [2021-2024_saber-pro-genericas_faedis.rds]({{< var url.data.saber_pro_faedis_rds >}})
:::
...
1. **Datos**: es el insumo principal que se requiere para generar un gráfico donde se refiere al conjunto de datos que se busca visualizar. `ggplot2` funciona mejor si el conjunto de datos es ordenado ([tidy data]({{< var url.packages.tidyr >}}) en inglés, [@wickham_tidy_2014]) donde este concepto se tratará en más detalle en el @sec-data-tidying.   
...
3. **Capas**: es el componente que toma los datos mapeados para transformarlos en una representación visual comprensible. Cada capa se compone de 3 elementos:
   ...
   a. Una **geometría** que determina cómo se muestran los datos a través de atributos estéticos.

   b. Una **transformación estadística** donde calcula nuevas variables a partir de los datos y afecta qué parte de ellos se muestra.
...
6. **Coordenadas**: es el componente que determina cómo se combinan los atributos estéticos `x` y `y` para posicionar los elementos en el plano del gráfico. Por defecto, se utiliza el sistema de coordenadas cartesianas, pero existen [otras opciones]({{< var url.packages.ggplot2_coord >}}) dentro de `ggplot2`.
...
```{r}
#| lst-label: lst-import-data
#| lst-cap: Importación de datos para obtener el insumo principal que se requiere para generar un gráfico
#| eval: false
# Importar datos ----
saber_pro_genericas_faedis_2021_2024 <- read_rds(
  file = "data/2021-2024_saber-pro-genericas_faedis.rds"
...
::: {#nte-2-chap-6 .callout-note}
En el @sec-rds se abarcará con más detalle la importación de datos para archivos con una extensión `.rds` mediante la función `read_rds()`, teniendo en cuenta lo indicado en el @lst-import-data. Por ahora, el enfoque del @sec-amounts será la visualización de un conjunto de datos.
:::
...
#### Descripción de variables {#sec-variable-description}

Para examinar y en particular crear un gráfico a partir de un conjunto de datos es necesario entender a qué se refiere cada una de las variables que lo componen. En ese sentido, a continuación se describen cada una de las variables en `2021-2024_saber-pro-genericas_faedis.rds`:
...
8. **estu_exterior**: indica si el estudiante presenta la prueba Saber Pro en el exterior (`SI` ó `NO`) 
9. **estu_cod_depto_presentacion**: código del departamento de presentación del examen según codificación [DIVIPOLA]({{< var url.institution.dane_divipola >}})[^chap-6-footnote-1]
10. **estu_depto_presentacion**: nombre del departamento de presentación del examen
11. **estu_cod_mcpio_presentacion**: código del municipio de presentación del examen según codificación [DIVIPOLA]({{< var url.institution.dane_divipola >}})
12. **estu_mcpio_presentacion**: nombre del municipio de presentación del examen
13. **estu_snies_prgmacademico**: código SNIES[^chap-6-footnote-2] del programa académico que cursa el estudiante y relacionado con el examen Saber Pro que presenta
14. **estu_prgm_academico**: nombre del programa académico que cursa el estudiante y relacionado con el examen Saber Pro que presenta
15. **mod_competen_ciudada_punt**: puntaje del módulo de competencias ciudadanas obtenido por el estudiante (el rango posible es $[0, 300]$)
16. **mod_comuni_escrita_punt**: puntaje del módulo de comunicación escrita obtenido por el estudiante (el rango posible es $[0, 300]$)
17. **mod_ingles_punt**: puntaje del módulo de inglés obtenido por el estudiante (el rango posible es $[0, 300]$)
18. **mod_ingles_desem**: nivel de desempeño del módulo de inglés obtenido por el estudiante (el rango posible es $[A_1, A_2, B_1, B_2]$)
19. **mod_lectura_critica_punt**: puntaje del módulo de lectura crítica obtenido por el estudiante (el rango posible es $[0, 300]$)
20. **mod_razona_cuantitat_punt**: puntaje del módulo de razonamiento cuantitativo obtenido por el estudiante (el rango posible es $[0, 300]$)
21. **punt_global**: puntaje total obtenido por el estudiante en el grupo de competencias genéricas (este grupo comprenden los módulos de competencias ciudadanas, comunicación escrita, inglés, lectura crítica y razonamiento cuantitativo y la variable se refiere al promedio simple del puntaje de estos módulos redondeado al entero más cercano)
...
4. `subtitle = "Periodo: 2021-2024"`: permite señalar un subtítulo donde en este caso se indica el periodo al que se refieren los datos
