...
A medida que explores diferentes formas de visualizar un conjunto de datos entenderás en más detalle la gramática de gráficos en capas utilizando el paquete `ggplot2`. Por ahora se describirá de manera general las 7 capas que conforman este modelo mental (ver @fig-mental-model-layered-grammar-graphics) y que se utilizan en `ggplot2` como un conjunto de instrucciones para generar un gráfico.
...
4. **Escalas**: es el componente que traduce la información visual del gráfico de vuelta a los datos originales, utilizando los ejes y las leyendas como guías.
   ...
   Sin embargo, existe también funciones auxiliares como `labs()` y `lims()`que permiten realizar ajustes a las etiquetas y los límites de un gráfico.
