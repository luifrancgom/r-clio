...
En caso que este utilizando Posit Workbench, de acuerdo a lo señalado en la @sec-posit-workbench, es posible que el proceso descrito a continuación no debas realizarlo. En algunas ocasiones el administrador de esta herramienta puede haber instalado ciertos paquetes con anterioridad.
...
Para entender cómo utilizar un paquete, explorarás tu primer conjunto de datos en R utilizando el paquete `tidyverse`. Primero, cierra la ventana de Positron si seguiste las instrucciones descritas en la @sec-how-to-install-r-positron y abre la carpeta `r-analisis-datos` (ver @sec-project-oriented-workflow-positron-r) utilizando las opciones **File > Open Folder...** que podrás encontrar en el menú superior izquierdo de la interfaz. En caso que estes utilizando Posit Workbench (ver @sec-posit-workbench) abre una sesión con el proyecto `r-analisis-datos`, creado de acuerdo a lo señalado en la @sec-project-oriented-workflow-positron-r.
...
Si ejecutas éste código, en la **Barra lateral secundaria** (ver @fig-positron-layout-outline) dentro de la pestaña **HELP**, se desplegará la documentación relacionada con `mpg`.
...
Por último, en ciertos contextos puede ser útil extraer y explorar sólo una variable dentro de un conjunto de datos. En R existen diversas formas de lograr esto. Sin embargo, señalaremos sólo 2 alternativas, enfocándonos en el uso del operador `$`, que está incluído por defecto en R, y la función `pull()` del paquete `dplyr`, que hace parte del paquete `tidyverse`.
